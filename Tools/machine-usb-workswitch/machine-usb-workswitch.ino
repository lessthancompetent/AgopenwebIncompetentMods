  ...
  #include <EEPROM.h> 
  // ---- AgOpenWeb fork: machine board WITH a work switch ----
// Stock Machine_USB_v5 has no work-switch input (that lives on the steer board
// in stock AOG). This fork reads one on A0 (INPUT_PULLUP, switch closes to
// ground - the natural wiring for an implement-mounted switch, since this
// board already sits on the implement) and reports it in the FromMachine
// frame's spare byte: AOG[8] bit 7 = "I have a work switch", bit 0 = raw
// closed state. Stock AgOpenGPS ignores that byte, so this stays compatible.
#define EEP_Ident 0x5400  
  ...
  void setup()
  {
      ...
      pinMode(12, OUTPUT);
      pinMode(13, OUTPUT);
      pinMode(A0, INPUT_PULLUP);   // work switch, closes to GND
  }
  ...
  void loop()
  {
      ...
      if (currentTime - lastTime >= LOOP_TIME)
      {
          ...
          // bit 7: this firmware reports a work switch; bit 0: closed to GND
          AOG[8] = 0x80 | ((digitalRead(A0) == LOW) ? 0x01 : 0x00);
