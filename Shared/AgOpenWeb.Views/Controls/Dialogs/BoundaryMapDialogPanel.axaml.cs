...
public partial class BoundaryMapDialogPanel : UserControl
{
    ...
    private AgOpenWeb.ViewModels.MainViewModel? _trackedVm;
    ...
    {
        if (e.PropertyName is nameof(AgOpenWeb.ViewModels.MainViewModel.Latitude)
                           or nameof(AgOpenWeb.ViewModels.MainViewModel.Longitude))
        {
        ...
        double MercMinX, double MercMaxX, double MercMinY, double MercMaxY)?> CaptureBackgroundImageAsync()
    {
        try
        {
            ...
            httpClient.DefaultRequestHeaders.UserAgent.ParseAdd("Mozilla/5.0 (compatible; AgOpenWeb/1.0)");
            ...
            var tempDir = Path.Combine(Path.GetTempPath(), "AgOpenWeb_Mapsui");
