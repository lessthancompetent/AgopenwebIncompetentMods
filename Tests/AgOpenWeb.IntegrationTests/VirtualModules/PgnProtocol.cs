...
public static class PgnProtocol
{
    ...
    public static AutoSteerCommand ParseAutoSteerCommand(byte[] data)
    {
        return new AutoSteerCommand
        {
            ...
            // Real firmware reads the whole byte as guidanceStatus — any nonzero
            // engages. Mirror that exactly so host bugs (flag bits leaking into
            // this byte) are CAUGHT by the virtual module, not masked.
            IsEngaged = data[7] != 0,
        };
    }
