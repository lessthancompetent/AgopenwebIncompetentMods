...
/// <summary>
/// Bug regression: when autosteer is disengaged (manual toggle, boundary
/// kickout, origin guard) the rendered U-turn arc must clear. The state
/// machine's auto-tick is gated on autoSteerEngaged, so without an explicit
...
public class YouTurnDisengageClearTests
{
    ...
    [Test]
    public void Cycle_WithAutoSteerDisengaged_ClearsActiveYouTurnSnapshot()
    {
        ...
        // Autosteer is off — this is the bug: the user disengaged mid-turn.
        ...
        Assert.Multiple(() =>
        {
            Assert.That(Last.YouTurn!.IsTriggered, Is.False,
                "Disengaging autosteer must clear IsTriggered so the map stops drawing the arc");
            Assert.That(Last.YouTurn!.IsExecuting, Is.False,
                "Disengaging autosteer must clear IsExecuting so SetIsInYouTurn(false) propagates");
            Assert.That(Last.YouTurn!.TurnPath, Is.Null,
                "Disengaging autosteer must clear TurnPath so SetYouTurnPath(null) propagates");
        });
    }

    [Test]
    public void Cycle_WithAutoSteerEngaged_DoesNotForceClearYouTurn()
    {
        // Autosteer engaged — the disengage-clear path must NOT fire. (No track
        // is set, so the auto-tick won't reach in either; this exercises only
        // the clear-on-disengage branch we added.) Also enable YouTurn so the
        // sibling clear-on-disable block stays quiet too.
        ...
        Assert.Multiple(() =>
        {
            Assert.That(Last.YouTurn!.IsExecuting, Is.True,
                "With autosteer engaged the disengage-clear must not fire");
            Assert.That(Last.YouTurn!.TurnPath, Is.Not.Null,
                "With autosteer engaged the seeded turn path must survive the cycle");
        });
    }
}
