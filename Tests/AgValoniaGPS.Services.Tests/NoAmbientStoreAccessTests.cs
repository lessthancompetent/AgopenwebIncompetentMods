...
public class NoAmbientStoreAccessTests
{
    ...
    public void NoStaticInstanceAccess_InServicesOrViewModels()
    {
        ...
        var scanDirs = new[]
        {
            Path.Combine(root, "Shared", "AgValoniaGPS.Services"),
            Path.Combine(root, "Shared", "AgValoniaGPS.ViewModels"),
        };
    ...
    private static string FindRepoRoot()
    {
        ...
        while (dir != null && !File.Exists(Path.Combine(dir.FullName, "AgValoniaGPS.sln")))
            dir = dir.Parent;
        Assert.That(dir, Is.Not.Null, "Could not locate repo root (AgValoniaGPS.sln) from test base dir.");
        return dir!.FullName;
    }
}
