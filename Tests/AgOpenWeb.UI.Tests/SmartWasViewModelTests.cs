...
public class SmartWasViewModelTests
{
    ...
    private SmartWasViewModel BuildVm()
    {
        ...
        return new SmartWasViewModel(_smartWas, _configService, _udpService, _autoSteer, new AgOpenWeb.Services.Threading.InlineUiDispatcher());
    }
    ...
    public void CloseSmartWasDialogCommand_RestoresActiveDialogToNone()
    {
        ...
        Assert.That(mvm.State.UI.ActiveDialog, Is.EqualTo(AgOpenWeb.Models.State.DialogType.SmartWas));

        mvm.CloseSmartWasDialogCommand?.Execute(null);
        Assert.That(mvm.State.UI.ActiveDialog, Is.EqualTo(AgOpenWeb.Models.State.DialogType.None));
    }
}
